import {
  __commonJS
} from "./chunk-HM4MQYWN.js";

// node_modules/no-profanity/lib/en.js
var require_en = __commonJS({
  "node_modules/no-profanity/lib/en.js"(exports, module) {
    var list = [
      "*damn",
      "*dyke",
      "*fuck*",
      "*shit*",
      "2 girls 1 cup",
      "2g1c",
      "4r5e",
      "5h1t",
      "5hit",
      "@$$",
      "a55",
      "a_s_s",
      "acrotomophilia",
      "ahole",
      "alabama hot pocket",
      "alaskan pipeline",
      "amcik",
      "anal",
      "andskota",
      "anilingus",
      "anus",
      "apeshit",
      "ar5e",
      "arrse",
      "arschloch",
      "arse",
      "arse*",
      "arsehole",
      "ash0le",
      "ash0les",
      "asholes",
      "ass",
      "ass monkey",
      "ass-fucker",
      "ass-hat",
      "ass-pirate",
      "assbag",
      "assbandit",
      "assbanger",
      "assbite",
      "assclown",
      "asscock",
      "asscracker",
      "asses",
      "assface",
      "assfucker",
      "assfukka",
      "assgoblin",
      "assh0le",
      "assh0lez",
      "asshat",
      "asshead",
      "asshole",
      "assholes",
      "assholz",
      "asshopper",
      "assjacker",
      "asslick",
      "asslicker",
      "assmonkey",
      "assmunch",
      "assmuncher",
      "asspirate",
      "assrammer",
      "assshole",
      "asssucker",
      "asswad",
      "asswhole",
      "asswipe",
      "auto erotic",
      "autoerotic",
      "ayir",
      "azzhole",
      "b!+ch",
      "b!tch",
      "b00b*",
      "b00bs",
      "b17ch",
      "b1tch",
      "babeland",
      "baby batter",
      "baby juice",
      "ball gag",
      "ball gravy",
      "ball kicking",
      "ball licking",
      "ball sack",
      "ball sucking",
      "ballbag",
      "balls",
      "ballsack",
      "bampot",
      "bangbros",
      "bareback",
      "barely legal",
      "barenaked",
      "bassterds",
      "bastard",
      "bastardo",
      "bastards",
      "bastardz",
      "basterds",
      "basterdz",
      "bastinado",
      "bbw",
      "bdsm",
      "beaner",
      "beaners",
      "beastial",
      "beastiality",
      "beastility",
      "beaver cleaver",
      "beaver lips",
      "bellend",
      "bestial",
      "bestiality",
      "bi+ch",
      "bi7ch",
      "biatch",
      "big black",
      "big breasts",
      "big knockers",
      "big tits",
      "bimbos",
      "birdlock",
      "bitch",
      "bitch*",
      "bitcher",
      "bitchers",
      "bitches",
      "bitchin",
      "bitching",
      "black cock",
      "blonde action",
      "blonde on blonde action",
      "bloody",
      "blow job",
      "blow your load",
      "blowjob",
      "blowjobs",
      "blue waffle",
      "blumpkin",
      "boffing",
      "boiolas",
      "bollock",
      "bollock*",
      "bollocks",
      "bollok",
      "bollox",
      "bondage",
      "boner",
      "boob",
      "boobie",
      "boobs",
      "booobs",
      "boooobs",
      "booooobs",
      "booooooobs",
      "booty call",
      "breasts",
      "brown showers",
      "brunette action",
      "buceta",
      "bugger",
      "bukkake",
      "bulldyke",
      "bullet vibe",
      "bullshit",
      "bum",
      "bung hole",
      "bunghole",
      "bunny fucker",
      "busty",
      "butt",
      "butt-pirate",
      "buttcheeks",
      "butthole",
      "buttmunch",
      "buttplug",
      "buttwipe",
      "c0ck",
      "c0cks",
      "c0cksucker",
      "c0k",
      "cabron",
      "camel toe",
      "camgirl",
      "camslut",
      "camwhore",
      "carpet muncher",
      "carpetmuncher",
      "cawk",
      "cawks",
      "cazzo",
      "chinc",
      "chink",
      "choad",
      "chocolate rosebuds",
      "chode",
      "chraa",
      "chuj",
      "cipa",
      "circlejerk",
      "cl1t",
      "cleveland steamer",
      "clit",
      "clitface",
      "clitoris",
      "clits",
      "clover clamps",
      "clusterfuck",
      "cnts",
      "cntz",
      "cnut",
      "cock",
      "cock*",
      "cock-head",
      "cock-sucker",
      "cockbite",
      "cockburger",
      "cockface",
      "cockhead",
      "cockjockey",
      "cockknoker",
      "cockmaster",
      "cockmongler",
      "cockmongruel",
      "cockmonkey",
      "cockmunch",
      "cockmuncher",
      "cocknose",
      "cocknugget",
      "cocks",
      "cockshit",
      "cocksmith",
      "cocksmoker",
      "cocksuck",
      "cocksuck ",
      "cocksucked",
      "cocksucked ",
      "cocksucker",
      "cocksucking",
      "cocksucks ",
      "cocksuka",
      "cocksukka",
      "cok",
      "cokmuncher",
      "coksucka",
      "coochie",
      "coochy",
      "coon",
      "coons",
      "cooter",
      "coprolagnia",
      "coprophilia",
      "cornhole",
      "cox",
      "crap",
      "creampie",
      "cum",
      "cumbubble",
      "cumdumpster",
      "cumguzzler",
      "cumjockey",
      "cummer",
      "cumming",
      "cums",
      "cumshot",
      "cumslut",
      "cumtart",
      "cunilingus",
      "cunillingus",
      "cunnie",
      "cunnilingus",
      "cunt",
      "cunt*",
      "cuntface",
      "cunthole",
      "cuntlick",
      "cuntlick ",
      "cuntlicker",
      "cuntlicker ",
      "cuntlicking",
      "cuntlicking ",
      "cuntrag",
      "cunts",
      "cuntz",
      "cyalis",
      "cyberfuc",
      "cyberfuck ",
      "cyberfucked ",
      "cyberfucker",
      "cyberfuckers",
      "cyberfucking ",
      "d1ck",
      "d4mn",
      "dammit",
      "damn",
      "darkie",
      "date rape",
      "daterape",
      "daygo",
      "deep throat",
      "deepthroat",
      "dego",
      "dendrophilia",
      "dick",
      "dick*",
      "dickbag",
      "dickbeater",
      "dickface",
      "dickhead",
      "dickhole",
      "dickjuice",
      "dickmilk",
      "dickmonger",
      "dickslap",
      "dicksucker",
      "dickwad",
      "dickweasel",
      "dickweed",
      "dickwod",
      "dike",
      "dike*",
      "dild0",
      "dild0s",
      "dildo",
      "dildos",
      "dilld0",
      "dilld0s",
      "dingleberries",
      "dingleberry",
      "dink",
      "dinks",
      "dipshit",
      "dirsa",
      "dirty pillows",
      "dirty sanchez",
      "dlck",
      "dog style",
      "dog-fucker",
      "doggie style",
      "doggiestyle",
      "doggin",
      "dogging",
      "doggy style",
      "doggystyle",
      "dolcett",
      "domination",
      "dominatricks",
      "dominatrics",
      "dominatrix",
      "dommes",
      "donkey punch",
      "donkeyribber",
      "doochbag",
      "dookie",
      "doosh",
      "double dong",
      "double penetration",
      "douche",
      "douchebag",
      "dp action",
      "dry hump",
      "duche",
      "dumbshit",
      "dumshit",
      "dupa",
      "dvda",
      "dyke",
      "dziwka",
      "eat my ass",
      "ecchi",
      "ejackulate",
      "ejaculate",
      "ejaculated",
      "ejaculates ",
      "ejaculating ",
      "ejaculatings",
      "ejaculation",
      "ejakulate",
      "ekrem*",
      "ekto",
      "enculer",
      "enema",
      "erotic",
      "erotism",
      "escort",
      "eunuch",
      "f u c k",
      "f u c k e r",
      "f4nny",
      "f_u_c_k",
      "faen",
      "fag",
      "fag*",
      "fag1t",
      "fagbag",
      "faget",
      "fagg",
      "fagg0t",
      "fagg1t",
      "fagging",
      "faggit",
      "faggitt",
      "faggot",
      "faggs",
      "fagit",
      "fagot",
      "fagots",
      "fags",
      "fagtard",
      "fagz",
      "faig",
      "faigs",
      "fanculo",
      "fanny",
      "fannyflaps",
      "fannyfucker",
      "fanyy",
      "fart",
      "farted",
      "farting",
      "farty",
      "fatass",
      "fcuk",
      "fcuker",
      "fcuking",
      "fecal",
      "feces",
      "feck",
      "fecker",
      "feg",
      "felatio",
      "felch",
      "felcher",
      "felching",
      "fellate",
      "fellatio",
      "feltch",
      "female squirting",
      "femdom",
      "ficken",
      "figging",
      "fingerbang",
      "fingerfuck ",
      "fingerfucked ",
      "fingerfucker ",
      "fingerfuckers",
      "fingerfucking ",
      "fingerfucks ",
      "fingering",
      "fistfuck",
      "fistfucked ",
      "fistfucker ",
      "fistfuckers ",
      "fistfucking ",
      "fistfuckings ",
      "fistfucks ",
      "fisting",
      "fitt*",
      "flamer",
      "flange",
      "flikker",
      "flipping the bird",
      "fook",
      "fooker",
      "foot fetish",
      "footjob",
      "foreskin",
      "fotze",
      "frotting",
      "fu(*",
      "fuck",
      "fuck buttons",
      "fucka",
      "fucked",
      "fucker",
      "fuckers",
      "fuckhead",
      "fuckheads",
      "fuckin",
      "fucking",
      "fuckings",
      "fuckingshitmotherfucker",
      "fuckme ",
      "fucks",
      "fucktards",
      "fuckwhit",
      "fuckwit",
      "fudge packer",
      "fudgepacker",
      "fuk",
      "fuk*",
      "fukah",
      "fuken",
      "fuker",
      "fukin",
      "fukk",
      "fukkah",
      "fukken",
      "fukker",
      "fukkin",
      "fuks",
      "fukwhit",
      "fukwit",
      "futanari",
      "futkretzn",
      "fux",
      "fux0r",
      "g-spot",
      "g00k",
      "gang bang",
      "gangbang",
      "gangbanged",
      "gangbanged ",
      "gangbangs ",
      "gay sex",
      "gayass",
      "gaybob",
      "gaydo",
      "gaylord",
      "gaysex",
      "gaytard",
      "gaywad",
      "genitals",
      "giant cock",
      "girl on",
      "girl on top",
      "girls gone wild",
      "goatcx",
      "goatse",
      "god damn",
      "god-dam",
      "god-damned",
      "goddamn",
      "goddamned",
      "gokkun",
      "golden shower",
      "goo girl",
      "gooch",
      "goodpoop",
      "gook",
      "goregasm",
      "gringo",
      "grope",
      "group sex",
      "guido",
      "guiena",
      "guro",
      "h00r",
      "h0ar",
      "h0r",
      "h0re",
      "h4x0r",
      "hand job",
      "handjob",
      "hard core",
      "hardcore",
      "hardcoresex ",
      "heeb",
      "hell",
      "hells",
      "helvete",
      "hentai",
      "heshe",
      "ho",
      "hoar",
      "hoare",
      "hoe",
      "hoer",
      "hoer*",
      "homo",
      "homoerotic",
      "honkey",
      "honky",
      "hooker",
      "hoor",
      "hoore",
      "hore",
      "horniest",
      "horny",
      "hot carl",
      "hot chick",
      "hotsex",
      "how to kill",
      "how to murder",
      "huevon",
      "huge fat",
      "hui",
      "humping",
      "incest",
      "injun",
      "intercourse",
      "jack off",
      "jack-off ",
      "jackass",
      "jackoff",
      "jail bait",
      "jailbait",
      "jap",
      "japs",
      "jelly donut",
      "jerk off",
      "jerk-off",
      "jerk-off ",
      "jigaboo",
      "jiggaboo",
      "jiggerboo",
      "jisim",
      "jism",
      "jiss",
      "jiz",
      "jiz ",
      "jizm",
      "jizm ",
      "jizz",
      "juggs",
      "kanker*",
      "kawk",
      "kike",
      "kinbaku",
      "kinkster",
      "kinky",
      "kiunt",
      "klootzak",
      "knob",
      "knobbing",
      "knobead",
      "knobed",
      "knobend",
      "knobhead",
      "knobjocky",
      "knobjokey",
      "knobs",
      "knobz",
      "knulle",
      "kock",
      "kondum",
      "kondums",
      "kooch",
      "kootch",
      "kraut",
      "kuk",
      "kuksuger",
      "kum",
      "kumer",
      "kummer",
      "kumming",
      "kums",
      "kunilingus",
      "kunt",
      "kunts",
      "kuntz",
      "kurac",
      "kurwa",
      "kusi*",
      "kyke",
      "kyrpa*",
      "l3i+ch",
      "l3itch",
      "labia",
      "leather restraint",
      "leather straight jacket",
      "lemon party",
      "lesbo",
      "lezzian",
      "lezzie",
      "lipshits",
      "lipshitz",
      "lmfao",
      "lolita",
      "lovemaking",
      "lust",
      "lusting",
      "m0f0",
      "m0fo",
      "m45terbate",
      "ma5terb8",
      "ma5terbate",
      "make me come",
      "male squirting",
      "mamhoon",
      "masochist",
      "masokist",
      "massterbait",
      "masstrbait",
      "masstrbate",
      "master-bate",
      "masterb8",
      "masterbaiter",
      "masterbat*",
      "masterbat3",
      "masterbate",
      "masterbates",
      "masterbation",
      "masterbations",
      "masturbat*",
      "masturbate",
      "menage a trois",
      "merd*",
      "mibun",
      "milf",
      "minge",
      "missionary position",
      "mo-fo",
      "mof0",
      "mofo",
      "monkleigh",
      "motha fucker",
      "motha fuker",
      "motha fukkah",
      "motha fukker",
      "mothafuck",
      "mothafucka",
      "mothafuckas",
      "mothafuckaz",
      "mothafucked ",
      "mothafucker",
      "mothafuckers",
      "mothafuckin",
      "mothafucking ",
      "mothafuckings",
      "mothafucks",
      "mother fucker",
      "mother fukah",
      "mother fuker",
      "mother fukkah",
      "mother fukker",
      "mother-fucker",
      "motherfuck",
      "motherfucked",
      "motherfucker",
      "motherfuckers",
      "motherfuckin",
      "motherfucking",
      "motherfuckings",
      "motherfuckka",
      "motherfucks",
      "mouliewop",
      "mound of venus",
      "mr hands",
      "muff",
      "muff diver",
      "muffdiver",
      "muffdiving",
      "muie",
      "mulkku",
      "muschi",
      "mutha",
      "mutha fucker",
      "mutha fukah",
      "mutha fuker",
      "mutha fukkah",
      "mutha fukker",
      "muthafecker",
      "muthafuckker",
      "muther",
      "mutherfucker",
      "n1gga",
      "n1gger",
      "n1gr",
      "nambla",
      "nastt",
      "nawashi",
      "nazi",
      "nazis",
      "negro",
      "neonazi",
      "nepesaurio",
      "nig nog",
      "nigg3r",
      "nigg4h",
      "nigga",
      "niggah",
      "niggas",
      "niggaz",
      "nigger",
      "nigger*",
      "nigger;",
      "niggers ",
      "niglet",
      "nigur;",
      "niiger;",
      "niigr;",
      "nimphomania",
      "nipple",
      "nipples",
      "nob",
      "nob jokey",
      "nobhead",
      "nobjocky",
      "nobjokey",
      "nsfw images",
      "nude",
      "nudity",
      "numbnuts",
      "nutsack",
      "nympho",
      "nymphomania",
      "octopussy",
      "omorashi",
      "one cup two girls",
      "one guy one jar",
      "orafis",
      "orgasim",
      "orgasim ",
      "orgasim;",
      "orgasims ",
      "orgasm",
      "orgasms ",
      "orgasum",
      "orgy",
      "oriface",
      "orifice",
      "orifiss",
      "orospu",
      "p0rn",
      "packi",
      "packie",
      "packy",
      "paedophile",
      "paki",
      "pakie",
      "paky",
      "panooch",
      "panties",
      "panty",
      "paska*",
      "pawn",
      "pecker",
      "peckerhead",
      "pedobear",
      "pedophile",
      "peeenus",
      "peeenusss",
      "peenus",
      "pegging",
      "peinus",
      "pen1s",
      "penas",
      "penis",
      "penis-breath",
      "penisfucker",
      "penus",
      "penuus",
      "perse",
      "phone sex",
      "phonesex",
      "phuc",
      "phuck",
      "phuk",
      "phuked",
      "phuker",
      "phuking",
      "phukked",
      "phukker",
      "phukking",
      "phuks",
      "phuq",
      "picka",
      "piece of shit",
      "pierdol*",
      "pigfucker",
      "pillu*",
      "pimmel",
      "pimpis",
      "pis",
      "pises",
      "pisin",
      "pising",
      "pisof",
      "piss",
      "piss pig",
      "piss*",
      "pissed",
      "pisser",
      "pissers",
      "pisses ",
      "pissflap",
      "pissflaps",
      "pissin",
      "pissin ",
      "pissing",
      "pissoff",
      "pissoff ",
      "pisspig",
      "pizda",
      "playboy",
      "pleasure chest",
      "polac",
      "polack",
      "polak",
      "pole smoker",
      "polesmoker",
      "pollock",
      "ponyplay",
      "poo",
      "poof",
      "poon",
      "poonani",
      "poonany",
      "poontang",
      "poontsee",
      "poop",
      "poop chute",
      "poopchute",
      "porn",
      "porno",
      "pornography",
      "pornos",
      "pr0n",
      "pr1c",
      "pr1ck",
      "pr1k",
      "preteen",
      "prick",
      "pricks ",
      "prince albert piercing",
      "pron",
      "pthc",
      "pube",
      "pubes",
      "pula",
      "pule",
      "punanny",
      "punany",
      "punta",
      "pusies",
      "pusse",
      "pussee",
      "pussi",
      "pussies",
      "pussy",
      "pussylicking",
      "pussys ",
      "pusy",
      "puta",
      "puto",
      "puuke",
      "puuker",
      "qahbeh",
      "queaf",
      "queef",
      "queef*",
      "queerbait",
      "queerhole",
      "quim",
      "qweir",
      "raghead",
      "raging boner",
      "rape",
      "raping",
      "rapist",
      "rautenberg",
      "recktum",
      "rectum",
      "renob",
      "retard",
      "reverse cowgirl",
      "rimjaw",
      "rimjob",
      "rimming",
      "rosy palm",
      "rosy palm and her 5 sisters",
      "ruski",
      "rusty trombone",
      "s hit",
      "s&m",
      "s.o.b.",
      "s_h_i_t",
      "sadism",
      "sadist",
      "santorum",
      "scank",
      "scat",
      "schaffer",
      "scheiss*",
      "schlampe",
      "schlong",
      "schmuck",
      "scissoring",
      "screw",
      "screwing",
      "scroat",
      "scrote",
      "scrotum",
      "semen",
      "sex",
      "sexo",
      "sexy",
      "sh!+",
      "sh!t",
      "sh!t*",
      "sh1t",
      "sh1ter",
      "sh1ts",
      "sh1tter",
      "sh1tz",
      "shag",
      "shagger",
      "shaggin",
      "shagging",
      "sharmuta",
      "sharmute",
      "shaved beaver",
      "shaved pussy",
      "shemale",
      "shi+",
      "shibari",
      "shipal",
      "shit",
      "shit-ass",
      "shit-bag",
      "shit-bagger",
      "shit-brain",
      "shit-breath",
      "shit-cunt",
      "shit-dick",
      "shit-eating",
      "shit-face",
      "shit-faced",
      "shit-fit",
      "shit-head",
      "shit-heel",
      "shit-hole",
      "shit-house",
      "shit-load",
      "shit-pot",
      "shit-spitter",
      "shit-stain",
      "shitass",
      "shitbag",
      "shitbagger",
      "shitblimp",
      "shitbrain",
      "shitbreath",
      "shitcunt",
      "shitdick",
      "shite",
      "shiteating",
      "shited",
      "shitey",
      "shitface",
      "shitfaced",
      "shitfit",
      "shitfuck",
      "shitfull",
      "shithead",
      "shitheel",
      "shithole",
      "shithouse",
      "shiting",
      "shitings",
      "shitload",
      "shitpot",
      "shits",
      "shitspitter",
      "shitstain",
      "shitted",
      "shitter",
      "shitters ",
      "shittiest",
      "shitting",
      "shittings",
      "shitty",
      "shitty ",
      "shity",
      "shitz",
      "shiz",
      "shiznit",
      "shota",
      "shrimping",
      "shyt",
      "shyte",
      "shytty",
      "shyty",
      "skanck",
      "skank",
      "skankee",
      "skankey",
      "skanks",
      "skanky",
      "skeet",
      "skribz",
      "skurwysyn",
      "slag",
      "slanteye",
      "slut",
      "slutbag",
      "sluts",
      "slutty",
      "slutz",
      "smeg",
      "smegma",
      "smut",
      "snatch",
      "snowballing",
      "sodomize",
      "sodomy",
      "son-of-a-bitch",
      "spac",
      "sphencter",
      "spic",
      "spick",
      "spierdalaj",
      "splooge",
      "splooge moose",
      "spooge",
      "spread legs",
      "spunk",
      "strap on",
      "strapon",
      "strappado",
      "strip club",
      "style doggy",
      "suck",
      "sucks",
      "suicide girls",
      "suka",
      "sultry women",
      "swastika",
      "swinger",
      "t1tt1e5",
      "t1tties",
      "tainted love",
      "tard",
      "taste my",
      "tea bagging",
      "teets",
      "teez",
      "testical",
      "testicle",
      "testicle*",
      "threesome",
      "throating",
      "thundercunt",
      "tied up",
      "tight white",
      "tit",
      "titfuck",
      "tits",
      "titt",
      "titt*",
      "tittie5",
      "tittiefucker",
      "titties",
      "titty",
      "tittyfuck",
      "tittywank",
      "titwank",
      "tongue in a",
      "topless",
      "tosser",
      "towelhead",
      "tranny",
      "tribadism",
      "tub girl",
      "tubgirl",
      "turd",
      "tushy",
      "tw4t",
      "twat",
      "twathead",
      "twatlips",
      "twatty",
      "twink",
      "twinkie",
      "two girls one cup",
      "twunt",
      "twunter",
      "undressing",
      "upskirt",
      "urethra play",
      "urophilia",
      "v14gra",
      "v1gra",
      "va-j-j",
      "va1jina",
      "vag",
      "vag1na",
      "vagiina",
      "vagina",
      "vaj1na",
      "vajina",
      "venus mound",
      "viagra",
      "vibrator",
      "violet wand",
      "vittu",
      "vjayjay",
      "vorarephilia",
      "voyeur",
      "vullva",
      "vulva",
      "w00se",
      "w0p",
      "wang",
      "wank",
      "wank*",
      "wanker",
      "wanky",
      "wet dream",
      "wetback",
      "wetback*",
      "wh00r",
      "wh0re",
      "white power",
      "whoar",
      "whore",
      "wichser",
      "willies",
      "willy",
      "wop*",
      "wrapping men",
      "wrinkled starfish",
      "xrated",
      "xx",
      "xxx",
      "yaoi",
      "yed",
      "yellow showers",
      "yiffy",
      "zabourah",
      "zoophilia"
    ];
    module.exports = { list };
  }
});

// node_modules/no-profanity/lib/no-profanity.js
var require_no_profanity = __commonJS({
  "node_modules/no-profanity/lib/no-profanity.js"(exports, module) {
    var { list: baseList } = require_en();
    var isProfane = (args) => {
      const matches = containsProfanities(args);
      return matches.length > 0;
    };
    var containsProfanities = (args) => {
      const { testString, options, wordList } = parseArgs(args);
      if (wordList.length === 0)
        return [];
      const matches = [];
      const replacedWordList = wordList.map((word) => word.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"));
      const regex = new RegExp(`\\b(${replacedWordList.join("|")})\\b`, "ig");
      let match;
      while ((match = regex.exec(testString)) !== null) {
        matches.push({ word: match[0], index: match.index });
      }
      return matches;
    };
    var replaceProfanities = (args) => {
      const { testString, options } = parseArgs(args);
      const matches = containsProfanities(args);
      const replacement = options.replacement ?? "*";
      let newString = testString;
      matches.forEach((match) => {
        const { word, index } = match;
        const asterisks = replacement.repeat(word.length);
        newString = newString.substring(0, index) + asterisks + testString.substring(index + word.length);
      });
      return newString;
    };
    var parseArgs = (args) => {
      var _a, _b, _c, _d;
      let excludededSet = /* @__PURE__ */ new Set();
      if ((_a = args == null ? void 0 : args.options) == null ? void 0 : _a.excludes) {
        excludededSet = /* @__PURE__ */ new Set([...args.options.excludes]);
      }
      let wordList = [];
      if ((_b = args == null ? void 0 : args.options) == null ? void 0 : _b.emptyList) {
        wordList = [];
      } else {
        wordList = baseList.filter((word) => !excludededSet.has(word));
      }
      let options, testString;
      if (typeof args === "string") {
        testString = args;
        options = {};
      } else {
        ({ testString, options } = args);
        if (!options || typeof options !== "object")
          options = {};
      }
      if (typeof (options == null ? void 0 : options.list) === "object" && ((_c = options == null ? void 0 : options.list) == null ? void 0 : _c.length) > 0) {
        log("The list option is deprecated, please use includes instead.", args);
        options.includes = [...options.list ?? [], ...options.includes ?? []];
      }
      if (typeof (options == null ? void 0 : options.includes) === "object" && ((_d = options == null ? void 0 : options.includes) == null ? void 0 : _d.length) > 0) {
        wordList = [...wordList, ...options.includes];
      }
      if (options == null ? void 0 : options.regex) {
        log("The regex option is deprecated, please use preSanitize instead.", args);
        options.preSanitize = options.regex;
      }
      if (options == null ? void 0 : options.preSanitize) {
        testString = testString.replace(options.preSanitize, (options == null ? void 0 : options.preSanitizeReplacement) ?? "");
      }
      if (options == null ? void 0 : options.placeHolder) {
        log("The placeHolder option is deprecated, please use replacement instead.", args);
        options.replacement = options.placeHolder;
      }
      return { testString, options, wordList };
    };
    var getProfanities = (options = {}) => {
      const { wordList } = parseArgs(options);
      return wordList;
    };
    var log = (msg, args) => {
      var _a;
      if ((_a = args == null ? void 0 : args.options) == null ? void 0 : _a.surpressWarnings) {
        return;
      }
      console.log(msg);
    };
    module.exports = { isProfane, containsProfanities, replaceProfanities, getProfanities };
  }
});
export default require_no_profanity();
//# sourceMappingURL=no-profanity.js.map
